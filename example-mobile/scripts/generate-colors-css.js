#!/usr/bin/env node
import { readFileSync, mkdirSync, writeFileSync, watchFile, unwatchFile } from 'fs';
...
function buildColorsCss() {
  try {
    const content = readFileSync(COLORS_SOURCE, 'utf8');
    ...
    const rel = relative(process.cwd(), OUT_CSS);
    if (changed) {
...
function watchColors() {
  console.log(`[colors-css] Watching ${relative(process.cwd(), COLORS_SOURCE)}...`);

  let debounceTimer = null;
  watchFile(COLORS_SOURCE, { interval: 250 }, () => {
